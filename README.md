...
- **관리자 페이지(`/admin`)** — 추적 리그 동기화, 개별/전체 경기 재예측, 순위표
  갱신, 백테스트 실행을 수동으로 트리거할 수 있습니다. 로그인 없이 누구나 접근
  가능한 페이지이므로(아래 [알려진 한계](#알려진-한계) 참고), 공개 배포 시 필요하면
  접근 제어를 추가하세요.
...
- **관리자 페이지에 인증이 없습니다** — `/admin`의 동기화/재예측/추적 해제/
  백테스트 실행 버튼은 로그인 없이 누구나 호출할 수 있는 Edge Function을
  부릅니다. 공개 배포 시 필요하면 Supabase Auth 등으로 접근을 제한하세요.
